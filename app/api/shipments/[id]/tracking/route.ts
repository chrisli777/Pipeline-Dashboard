...
// Shipment-level transitions: ON_WATER→CLEARED, DELIVERED→CLOSED
// Container-level transitions: CLEARED→DELIVERING, DELIVERING→DELIVERED
// Shipment auto-derives DELIVERING/DELIVERED from container statuses
const VALID_TRANSITIONS: Record<string, string[]> = {
  'ON_WATER': ['CLEARED', 'DELIVERING'],  // DELIVERING = rare skip
  'CLEARED': ['DELIVERING'],
...
export async function PATCH(
  ...
  try {
    ...
    const {
      status,
      // Clearance fields (ON_WATER → CLEARED)
      cleared_date,
      duty_amount,
      entry_number,
      broker,
    ...
    // Note: status_history is automatically updated by the database trigger
    ...
    if (status === 'DELIVERED' && delivered_date) {
      try {
        ...
        // deliveryResults contains per-SKU delivery info (logged for debugging)
        ...
        // Don't fail the status update if inventory sync fails
